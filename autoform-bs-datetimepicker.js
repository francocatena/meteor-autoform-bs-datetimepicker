AutoForm.addInputType('bootstrap-datetimepicker', {
  ...
  valueIn: function (val, atts) {
    ...
    if (typeof timezoneId === 'string') {
      ...
      if (val instanceof Date) {
        ...
        return moment(AutoForm.Utility.dateToNormalizedLocalDateAndTimeString(val, timezoneId), format).toDate()
      }
    }
  ...
  valueOut: function () {
    var m = this.data('DateTimePicker') && this.data('DateTimePicker').date()
...
Template.afBootstrapDateTimePicker.rendered = function () {
  ...
  // the current datetime when we call date() on an empty field.
  if (!opts.defaultDate || opts.defaultDate === '') opts.defaultDate = null
  ...
  this.autorun(function () {
    ...
    // set field value
    if (data.value instanceof Date) dtp.date(data.value)

    // set start date if there's a min in the schema
    if (data.min instanceof Date) dtp.setMinDate(data.min)

    // set end date if there's a max in the schema
    if (data.max instanceof Date) dtp.setMaxDate(data.max)
  })
}
